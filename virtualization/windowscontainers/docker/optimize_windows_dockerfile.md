...
第一個範例會下載、擷取，並清除 Visual Studio 可轉散發套件。 每個動作都會依自己的 `RUN` 指令執行。
...
```
命令可以重新撰寫，因此一項 `RUN` 指令中的每一個作業都可以在本身的行中指定。 
...
[Windows 上的 Dockerfile] (./manage_windows_dockerfile.md)

[Docker.com 上寫入 Dockerfiles 的最佳做法](https://docs.docker.com/engine/reference/builder/)
