...
要點指南分別如下：
* [以 Docker 管理 Windows 容器](../quick_start/manage_docker.md)
* [以 PowerShell 管理 Windows 容器](../quick_start/manage_powershell.md) 
...
如果您用過 Hyper-V PowerShell，對於 Cmdlet 的設計應該很熟悉。 其許多工作流程類似於使用 Hyper-V 模組管理虛擬機器。 不是 `New-VM`、`Get-VM`、`Start-VM`、`Stop-VM`，您有 `New-Container`、`Get-Container`、`Start-Container`、`Stop-Container`。  有不少容器專用的 Cmdlet 和參數，但 Windows 容器的一般生命週期和管理看起來和 Hyper-V 虛擬機器的週期和管理相似。
...
PowerShell Cmdlet 並不是完美的同等對應，而且有很多的命令我們無法提供 PowerShell 替代品 * (尤其是 `docker build` 和 `docker cp`)。 不過最令您訝異的可能是 `docker run` 沒有單一行的 PowerShell 替代品。
...
2.  目前您無法在啟動容器時傳遞要在容器內執行的命令。 不過，您還可以使用 `Enter-PSSession -ContainerId <ID of a running container>` 取得執行中容器的互動式 PowerShell 工作階段，也可以使用 `Invoke-Command -ContainerId <container id> -ScriptBlock { code to run inside the container }` 或 `Invoke-Command -ContainerId <container id> -FilePath <path to script>` 在執行的容器內執行命令。  
這兩個命令都可使用選擇性的 `-RunAsAdministrator` 旗標來進行高權限的動作。  
...
### 建立您自己的範例
您可以使用 `Get-Command -Module Containers` 來查看所有的容器 Cmdlet。  其他幾個這裡沒有描述的 Cmdlet，留待您自己去了解。    
**注意**：這不會傳回 `Enter-PSSession` 和 `Invoke-Command`，這兩個 Cmdlet 屬於核心 PowerShell。

您也可以使用 `Get-Help [cmdlet name]` 或對等的 `[cmdlet name] -?` 來取得任何 Cmdlet 的說明。  目前，「說明」輸出是自動產生，只是告訴您命令的語法，隨著 Cmdlet 的設計越來越接近完成，我們將更進一步加入文件。
...
備註：只是為了證明可以這麼做，以下的 PowerShell 函式由我們已介紹過的一些 Cmdlet 組成，用在假造的 `docker run`。 (先說清楚，這是概念證明，不是真的開發。)
...
為了不與 Docker 的 API 文件重複，您可從<a href="https://docs.docker.com/engine/reference/commandline/cli/" >這裡</a>連結至 Docker 管理 API。  他們的逐步解說很棒。
