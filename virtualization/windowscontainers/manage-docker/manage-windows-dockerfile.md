...
如需的 Dockerfiles Windows 的其他範例，請參閱 [Dockerfile for Windows 存放庫] (https://github.com/Microsoft/Virtualization-Documentation/tree/master/windows-container-samples)。

## <a name="instructions"></a>指示

Dockerfile 指令為 Docker 引擎提供建立容器映像所需的步驟。 這些指令會依序逐一執行。 以下是一些基本 Dockerfile 指令的詳細資料。 Dockerfile 指示的完整清單，請參閱 [Dockerfile 參考 （英文） 上 Docker.com] (https://docs.docker.com/engine/reference/builder/)。
