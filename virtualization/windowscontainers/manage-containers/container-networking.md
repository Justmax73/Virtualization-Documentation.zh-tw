...
## 搭配 Docker 網路的 Switch Embedded Teaming (交換器內嵌小組)
...
虛擬篩選平台 (VFP) 擴充功能是一種 Hyper-V 虛擬交換器，可轉送用來強制執行網路原則及操控封包的擴充功能。 例如，'overlay' 網路驅動程式會使用 VFP 來執行 VXLAN 封裝，而 'l2bridge' 驅動程式會用它在輸入和輸出時執行 MAC 重寫。 只有 Windows Server 2016 和 Windows 10 Creators Update 上有 VFP 擴充功能。 若要查看其是否正常執行，使用者可以執行兩個命令：
...
#### Moby Linux VM 使用 DockerNAT 交換器來搭配 Docker for Windows ([Docker CE](https://www.docker.com/community-edition) 的產品)，而不是使用 HNS 內部 vSwitch 
...
#### 若要在虛擬容器主機上使用 DHCP 來進行 IP 指派，請啟用 MACAddressSpoofing 
...
#### 使用靜態 IP 指派時，請記得要指定 *--subnet* 和 *--gateway*
...
#### 現有的 vSwitch (Docker 看不到) 會阻止建立透明網路
