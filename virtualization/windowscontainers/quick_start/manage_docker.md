...
建立容器之前，請使用 `docker images` 命令列出主機上安裝的容器映像。
...
最後，將會使用 `docker commit` 將容器認可至新的容器映像。 此範例會使用名稱 `windowsservercoreiis` 建立新的容器映像。
...
完成之後，您可以使用 `docker images` 命令來確認映像已建立。
...
由於容器將會執行 Nano Server OS 映像，因此必須要有 Nano Server IIS 封裝，才能安裝 IIS。 這些項目可在 Windows Sever 2016 TP4 安裝媒體中的 `NanoServer\Packages` 目錄下找到。
