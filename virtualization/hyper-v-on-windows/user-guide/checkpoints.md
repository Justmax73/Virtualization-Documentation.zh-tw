...
* **標準檢查點**： 的虛擬機器和虛擬機器記憶體狀態的快照在起始檢查點時的時間。 快照不是完整備份，而且可能導致在不同節點 (例如 Active Directory) 之間複寫資料的系統發生不一致問題。  Hyper-V 只在 Windows 10 之前提供標準檢查點 (以前稱為快照)。

* **生產檢查點**： 使用磁碟區陰影複製服務或檔案系統凍結 Linux 虛擬機器上建立資料一致的虛擬機器備份。 系統並不會建立虛擬機器記憶體狀態的任何快照。
...
1.  在 **\[Hyper-V 管理員\]** 中，選取虛擬機器。
2.  在**檢查點**區段中，以滑鼠右鍵按一下您想要刪除，然後按一下 \ [刪除檢查點。 您也可以刪除某個檢查點和所有後續的檢查點。 若要這樣做，請以滑鼠右鍵按一下您要刪除的最早檢查點，然後按一下 **\[刪除檢查點樹狀子目錄\]**。
...
1. 登入虛擬機器，並在桌面上建立新的文字檔。 如果您依照先前的練習，您可以使用現有的文字檔。
