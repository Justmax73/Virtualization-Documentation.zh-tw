...
如果您正在膝上型電腦上執行 Windows 10 HYPER-V 且經常切換使用無線網路與有線的網路時，您可能會想要建立虛擬交換器乙太網路與無線網路卡。  取決於如何膝上型電腦連線到網路，您可以變更您在這些交換器之間的虛擬機器。 虛擬機器沒有有線和無線之間切換自動。 

>[!IMPORTANT]
>兩種方式切換不支援透過無線網路卡的外部 vSwitch，應該用於僅供測試。
