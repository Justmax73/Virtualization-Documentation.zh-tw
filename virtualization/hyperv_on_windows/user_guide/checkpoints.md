...
3.  系統可能會要求您確認是否要刪除檢查點。 在確認那是正確的檢查點後，按一下 **[刪除]**。
...
檢查點設定檔的預設儲存位置是：`%systemroot%\ProgramData\Microsoft\Windows\Hyper-V\Snapshots`。
...
2. 以 [記事本] 開啟此檔案，並輸入文字「這是標準檢查點」。 **不要儲存檔案或關閉 [記事本]**。
...
2. 在文字檔中輸入「這是生產檢查點」， 儲存檔案但**不要關閉 [記事本]**。
